...
function MarkdownEditor() {
  ...
  const getCategory = async () => {
    try {
      const category = await axios.get(
        "https://guparesourcepack.duckdns.org/api/wiki/get/title"
      );
  ...
  const getContent = async (wiki_Title) => {
    const res = await axios.get(
      "https://guparesourcepack.duckdns.org/api/wiki/get/content",
      {
        params: {
  ...
  const handleAddCategory = () => {
    if (newCategory.trim() !== "" && !categories.includes(newCategory)) {
      axios
        .post("https://guparesourcepack.duckdns.org/api/wiki/reg/title", {
        ...
        <ul className="space-y-2">
          {Array.isArray(categories) && categories.length > 0 ? (
            categories.map((cat, idx) => (
              <li
                onClick={() => handleSelectCategory(cat.wiki_Title)}
                key={idx}
